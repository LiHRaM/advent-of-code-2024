...
fn findAntinodes(allocator: std.mem.Allocator, matrix: BoundedSparseMatrix) !Antinodes {
    ...
    for (matrix.inner.items(.v), 0..) |left, i| {
        for (matrix.inner.items(.v), 0..) |right, j| {
            ...
            // then place antinode 'A = (A - D)
            const a = A.place(D.reverse(), '#') orelse continue;
            if (matrix.contains(a)) try nodes.put(a, undefined);

            // and another node 'B = (B + D)
            const b = B.place(D, '#') orelse continue;
            if (matrix.contains(b)) try nodes.put(b, undefined);
        }
    }
...
test "antinodes" {
    ...
    try std.testing.expectEqual(1, antinodes.count());
}
...
test "example" {
    try std.testing.expectEqual(14, try process(EXAMPLE_DATA));
}
